...
select_stations <- function(df) {
  ...
  if (length(treeline_index) > 0) {
    ...
    treeline_NDVI <- df$NDVI[treeline_index[1]]
  ...
  # Add treeline_NDVI as a column to the selected dataframe
  selected_df$treeline_NDVI <- treeline_NDVI
  return(selected_df)
}
...
perform_regressions <- function(df, peak_name, direction, year, peak_id, longitude, latitude, country, treeline_NDVI) {
  if (!is.data.frame(df) || nrow(df) <= 1) {
    return(data.frame(
      ...
      Treeline_NDVI = rep(treeline_NDVI, 3)
    ))
  }
...
for (peak_name in names(selected_nested_peaks)) {
  ...
  for (direction in names(peak_data)) {
    ...
    for (year in 1:length(direction_data)) {
      ...
      treeline_NDVI = unique(year_data$treeline_NDVI)[1]
      ...
      regression_results <- perform_regressions(year_data, peak_name, direction, 
                                                ...
                                                latitude,country,treeline_NDVI )
...
write.csv(final_results_df,"Regressions_26Oct.csv")
```
